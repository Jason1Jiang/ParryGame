...
**调整内容**:
- ✅ 采用"快速死亡"预设方案
- ✅ 总动画时长: 2.2秒 → **1.2秒** (减少45%)
- ✅ 减少玩家等待时间，提升游戏节奏

**具体参数变化**:
```json
{
  "animationDuration": 2000 → 1000,     // 总时长减半
  "shrinkDuration": 400 → 200,          // 缩小时长减半
  "particleLifeMin": 500 → 300,         // 粒子寿命缩短
  "particleLifeMax": 800 → 500,         // 粒子寿命缩短
  "timeSlowDuration": 1000 → 500,       // 慢动作时长减半
  "fadeOutDelay": 2000 → 1000           // 淡出延迟减半
}
```
...
```
0ms     - 触发死亡（震动、慢动作、闪光、冲击波）
0-200ms - 玩家缩小旋转（1.0→0.3，0°→360°）⚡ 加快
200-500ms - 粒子爆散（高速飞散）⚡ 缩短
500-1000ms - 粒子消散（逐渐消失）⚡ 缩短
1000-1200ms - 淡入黑色遮罩（透明度0→0.8）⚡ 提前
1200ms - 显示结算界面 ✅ (总时长1.2秒，原2.2秒)
```
...
### 新增配置 (config.json) - 已优化为快速版本⚡
```json
"playerDeath": {
  "enabled": true,
  "animationDuration": 1000,  // ⚡ 优化：从2000减少到1000
  "shrinkDuration": 400,
  "shrinkScale": 0.3,
  "rotationSpeed": 360,
